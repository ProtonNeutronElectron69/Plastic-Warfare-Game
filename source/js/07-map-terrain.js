...
function makeMap(key,seed){
 ...
 if(key!=='desk')for(const s of M.starts){ // v35: Desk (survival-only) skips the PvP base/expansion economy; survivalSetup lays its own
  ...
  /* v107.1: on a sided map the three home nodes jitter 1.0 instead of 2.2. The
     Attic's wall stands on row 17 now and the forward pile (p2) sits at row ~14:
     jittered 2.2 it reached row 16.5, and a crate on row 17 refuses a node within
     2.8 (lvlOK), so the crate wall across a base's front lost one to three of its
     four pairs on half the seeds. Same two rnd() draws per node either way. */
  const hj=def.t2v2?1.0:undefined;
  jnode('plastic',p1.x,p1.y,2550,hj); jnode('plastic',p2.x,p2.y,2550,hj);
  jnode('battery',bt.x,bt.y,1950,hj);
  ...
  /* v107.1 (owner pass): on a SIDED map the natural aims OUTBOARD along the edge -
     away from the teammate - and sits closer (8-9.5 tiles out instead of
     11.5-13.5). The Attic's walls came in seven rows (FRONT 24 -> 17), and a
     natural aimed at the centre stood at rows 18-22, IN the wall band: every layer
     would have been breached at its pocket and the expansion would have been the
     doorway. The pick takes the same two rnd() draws per try, so nothing after it
     moves for that reason; the corner maps take the old aim untouched. */
  {const sided=!!def.t2v2;
   const dxc=sided?-dx*N:N/2-s.x,dyc=sided?0:N/2-s.y,L=dhyp(dxc,dyc)||1,px=-dyc/L,py=dxc/L;
   let ex=null;
   for(let k=0;k<12&&!ex;k++){
    const d0=(sided?8:11.5)+rnd()*(sided?1.5:2),side=(rnd()*2-1)*3;
   ...
   /* v107.1: and on a sided map a SECOND home site on the same flank, out toward
      the corner - the owner's "1 battery / 1 plastic pile expansion to the side of
      each starting base". A fixed ladder in y under the natural's own 4.2 node
      clearance, no rnd() of its own (expoSite draws the pair's angle as ever). It
      carries the mid expansions' amounts, not the natural's: it is the safest
      ground on the map. */
   if(sided){const fx=s.x-dx*17;let sx2=null;
    for(const oy of [0,2,-2,3,-3,4,-4]){const cy2=s.y+oy;if(M.nodes.every(nd=>dhyp(nd.x-fx,nd.y-cy2)>4.2)){sx2={x:fx,y:cy2};break}}
    if(sx2)expoSite(sx2.x,sx2.y,1400,1300,true);}}
 }
 ...
 /* v107.1: a HANDFUL more around the middle, for a map that asks (the Attic).
    mineField scatters over the whole board and the centre cache's five nodes
    refuse most of the ground around the middle (farNode 2.6), so the Attic
    measured 0-2 mines within ten tiles of its centre over 40 seeds, mean 1.8 -
    the owner asked for some in the middle "if there aren't any already there",
    and on some seeds there were none. Same rules as mineField (open tile, clear
    of nodes and pockets, three tiles from every other mine, the mirror tested
    too), picked in an annulus 4..10 around the centre, AFTER mineField so the
    scatter is what it always was and these are added to it. */
 const midMines=(pairs)=>{
  const farMine=(x,y)=>!M.mines.some(o=>Math.abs(o.x-x)<3&&Math.abs(o.y-y)<3);
  let placed=0,guard=0;
  while(placed<pairs&&guard++<200){
   const a=rnd()*6.283,r=4+rnd()*6,tx=Math.floor(c+dcos(a)*r),ty=Math.floor(c+dsin(a)*r),mx=tx+.5,my=ty+.5;
   if(M.pass[ty*N+tx]!==1||nearExpo(mx,my,4.5)||!farNode(mx,my,2.6)||!farMine(mx,my))continue;
   const m=mir(mx,my);
   if((Math.abs(mx-m.x)<3&&Math.abs(my-m.y)<3)||!farMine(m.x,m.y)||M.pass[Math.floor(m.y)*N+Math.floor(m.x)]!==1)continue;
   M.mines.push({x:mx,y:my,live:true},{x:m.x,y:m.y,live:true});
   placed++;
  }
 };
 ...
 if(key==='backyard'){
  ...
  /* a dropped towel (cosmetic) somewhere off the beaten path. v107.3: it is
     painted as CLOTH now (the `towel` flag, drawTowel in renderTerrain) rather
     than as a pale rectangle with an outline - on v107.2's warm porcelain it
     read as a blank white slab, which is the second half of the owner's note.
     The rolls are unchanged, so the map is byte-identical: same four rnd()
     draws, same position, same size. */
  patchAt(()=>({x:8+rnd()*(N-24),y:8+rnd()*(N-22),w:10+rnd()*4,h:7+rnd()*3,towel:1}));   // no fill/stroke/inset: drawTowel owns every tone, and a dead field is a field the next reader believes
  ...
  /* THE FRONT LINE, north side (the south side is its point mirror). FRONT is the
     row the INNERMOST layer stands on; behind it (smaller y) is the compound, and
     the other layers stack OUTWARD from it. v107 stood it on row 24 with three
     layers at most; v107.1 (owner pass) pulls it in seven rows to 17 and adds a
     layer to every stretch, so the wall is rows 17-23 instead of 24-28.
     Two things decide the shape of row 17. A hedgehog may not stand within ten
     tiles of a start (barrTile), and at this row that is the six tiles in front
     of each base - so those are CRATES, whose centre is a tile further out and
     clears. And the base's own nodes jitter as far as rows 16-18, so a stretch
     beside a battery is short a tile or a crate on some seeds; the layers outside
     it are what the wall is for.
     Gates, on every layer: the west one at x 14-17, the centre one at x 33-38 on
     row 17 and 31-40 on the layers outside it (the north mid expansion sits just
     OUTSIDE the wall now, at row ~22, and its pocket widens the gap on whichever
     layers it touches), the east one at x 54-57. Reading west to east: crates
     with two hedgehog layers outside them, gate, a crate wall across the west
     base's front under three hedgehog layers and hedgehogs out to the centre
     gate, gate, hedgehogs then a crate wall across the east base's front under
     three layers, gate, three hedgehog layers with a crate row between them. */
  const FRONT=17;
  for(const tx of [0,2,4,6,8,10,12])lvlPair(tx,FRONT);          // west stretch: solid crates to the rim
  hedgeRow(2,13,FRONT+3,2);hedgeRow(2,13,FRONT+6,3);            // ...with two hedgehog layers outside it
  for(const tx of [18,20,22,24])lvlPair(tx,FRONT);              // west base: a crate wall across its front...
  hedgeRow(27,32,FRONT,1);                                       // ...closed out to the centre gate by hedgehogs
  hedgeRow(18,30,FRONT+2,3);hedgeRow(18,30,FRONT+4,2);hedgeRow(18,30,FRONT+6,3); // three layers over it, holes staggered
  hedgeRow(39,44,FRONT,1);                                       // east base: hedgehogs from the centre gate...
  for(const tx of [46,48,50,52])lvlPair(tx,FRONT);              // ...then a crate wall across its front
  hedgeRow(41,53,FRONT+2,3);hedgeRow(41,53,FRONT+4,3);hedgeRow(41,53,FRONT+6,2); // three layers over it
  hedgeRow(58,69,FRONT,2);hedgeRow(58,69,FRONT+2,2);hedgeRow(58,69,FRONT+6,3);   // east stretch: three hedgehog layers...
  lvlPair(70,FRONT);for(const tx of [59,63,67])lvlPair(tx,FRONT+4); // ...the rim crate, and a crate row between them
 ...
 if(key!=='desk'){const lawn=key==='backyard';
  ...
  if(def.t2v2){mineField();midMines(MID_MINE_PAIRS);barrCluster(3);} // v107.1: plus the handful around the middle
...
function renderTerrain(){
 ...
 if(th==='bath'){
  /* v107.2 (owner pass) THE HEXAGON MOSAIC. v107 drew this floor the way the
     Kitchen Counter is drawn - a square grout grid with every other square a
     shade off - and the owner read the two as the same floor. They were: the
     only differences were the grid's pitch (2 tiles against 4) and a slightly
     brighter grey-blue. A pattern that merely runs at a different pitch is the
     same pattern.
     So the bathroom gets the floor a bathroom actually has: a mosaic of small
     hexagons, warm-white glaze, dark grout, with a scattered share of grey
     accent tiles. Nothing about the square tile grid can be confused with it -
     six sides meeting three to a corner is not a grid at any pitch.
     THE HEXES ARE LAID IN WORLD SPACE, NOT ON THE SCREEN. The projection is
     linear, so a regular world hexagon comes out a skewed screen hexagon that
     still tiles perfectly, and the mosaic lies DOWN with the board the way the
     kitchen's grout does. A screen-space hex grid would have been regular and
     would have read as a flat sticker over an isometric board.
     R is the hexagon's circumradius in world tiles; a hexagon is 1.73*R across
     the flats, so at .8 the mosaic runs finer than the two-tile grid it
     replaces, which is what a mosaic is. */
  const BR=.8, BSX=1.7320508*BR, BSY=1.5*BR;
  c.save();
  /* clipped to the slab: unlike a tile loop, a lattice does not stop at the
     board's edge by construction, and anything drawn past it hangs in the air
     off the side of the mat (which is what map_shot.sh's magenta rim is for). */
  c.beginPath();matCorners.forEach((p2,i)=>i?c.lineTo(p2[0],p2[1]):c.moveTo(p2[0],p2[1]));c.closePath();c.clip();
  const hexAt=(wx,wy,k)=>{c.beginPath();
   for(let i=0;i<6;i++){const a=1.5707963+i*1.0471976,px=wx+dcos(a)*BR*k,py=wy+dsin(a)*BR*k;
    const sx=isoX(px,py),sy=isoY(px,py);i?c.lineTo(sx,sy):c.moveTo(sx,sy);}
   c.closePath();};
  c.lineJoin='round';
  for(let j=-1,jn=Math.ceil(N/BSY)+1;j<=jn;j++)for(let i=-1,inn=Math.ceil(N/BSX)+1;i<=inn;i++){
   const wx=i*BSX+((j&1)?BSX*.5:0), wy=j*BSY;
   /* dth, the painters' own deterministic hash - never rnd(). A mosaic drawn
      off the terrain rng would shear against itself the day another painter is
      added above it, and the v97 rule already says detail in a painter must be
      deterministic. */
   const h=dth(i*13.7+j*57.3), h2=dth(j*29.1-i*7.9);
   const accent=h<.085;                       // the scattered grey tiles of a vintage floor
   c.fillStyle=accent?shade('#b9b2a7',.97+h2*.08):shade('#f4f0e8',.965+h2*.055);
   hexAt(wx,wy,.995);c.fill();
   // the glaze: a smaller hexagon set up and to the left, so each tile domes
   c.fillStyle='rgba(255,255,255,.165)';
   hexAt(wx-BR*.13,wy-BR*.13,.66);c.fill();
  }
  // the grout, walked as one stroke pass so a shared edge is drawn once per tile
  c.strokeStyle='rgba(122,110,96,.55)';c.lineWidth=1.5;
  for(let j=-1,jn=Math.ceil(N/BSY)+1;j<=jn;j++)for(let i=-1,inn=Math.ceil(N/BSX)+1;i<=inn;i++){
 ...
 if(G.map.tub){const tb=G.map.tub; // v107: the drained tub's floor - the rim is props. The oval is rotated in WORLD space, so it is walked as a polygon rather than drawn as a screen ellipse
  ...
  c.save();
  /* v107.3 (owner pass) THE TUB WAS BLOWN OUT, and the cause was arithmetic, not
     colour. Its floor was one radial gradient of radius `rx*1.414*HW` - 430px on
     a 9.5-tile oval whose painted screen radius is at most ~310px and, across
     the short axis, half that. So the whole basin sat inside the gradient's
     first stops and the dark end was never reached anywhere: three stops, one
     visible, a flat near-white disc. A GRADIENT WIDER THAN THE SHAPE IT FILLS
     HAS NO SHADING IN IT.
     Two fixes, and the first is the one to carry: every radius here is DERIVED
     from the polygon actually walked, so the shading tracks the tub whatever
     size or angle a future map gives it. And the tub is painted as a BASIN
     rather than as a disc - an inner wall in shadow, then the floor inside it -
     because a shape-following ring gives an even rim on an ellipse where a
     radial gradient cannot: hug the long ends and the short sides stay pale,
     hug the short sides and the ends clip to flat dark. */
  let RRx=0,RRy=0;
  for(let i=0;i<48;i++){const a=i/48*6.283,u=tb.rx*dcos(a),v=tb.ry*dsin(a);
   const wx=tb.cx+u*ct-v*st,wy=tb.cy+u*st+v*ct;
   RRx=Math.max(RRx,Math.abs(isoX(wx,wy)-cx2));RRy=Math.max(RRy,Math.abs(isoY(wx,wy)-cy2));}
  // the inner wall: lit across the far side, in shadow on the near one
  {const wg=c.createLinearGradient(0,cy2-RRy,0,cy2+RRy);
   wg.addColorStop(0,'#dfe8ed');wg.addColorStop(.45,'#c2d0d9');wg.addColorStop(1,'#9db0bd');
   c.fillStyle=wg;tubPath(.97);c.fill();}
  // the floor inside it, lit from the upper left, coming down to a real edge
  {const fr=Math.min(RRx,RRy)*.9;
   const fg=c.createRadialGradient(cx2-RRx*.22,cy2-RRy*.3,fr*.06,cx2,cy2,fr);
   fg.addColorStop(0,'#f4f8fa');fg.addColorStop(.55,'#e0e9ee');fg.addColorStop(1,'#bfced8');
   c.fillStyle=fg;tubPath(.78);c.fill();}
  c.strokeStyle='rgba(130,150,162,.55)';c.lineWidth=3;tubPath(.78);c.stroke(); // the tide line, on the wall/floor joint
  c.strokeStyle='rgba(150,170,182,.3)';c.lineWidth=1.4;tubPath(.88);c.stroke(); // a second, higher water line up the wall
  /* enamel wear: a few faint arcs off dth, never rnd() - the terrain stream is
     shared with every painter after this one, and a decoration must not move
     the decorations below it (the v97 deterministic-detail rule) */
  c.save();c.strokeStyle='rgba(120,142,156,.16)';c.lineWidth=1.2;
  for(let i=0;i<7;i++){const k=.30+dth(i*4.1+tb.cx)*.42,a0=dth(i*9.3+tb.cy)*6.283,sw=.5+dth(i*2.7)*1.1;
   c.beginPath();
   for(let j=0;j<=14;j++){const a=a0+sw*j/14,u=tb.rx*k*dcos(a),v=tb.ry*k*dsin(a);
    const wx=tb.cx+u*ct-v*st,wy=tb.cy+u*st+v*ct;j?c.lineTo(isoX(wx,wy),isoY(wx,wy)):c.moveTo(isoX(wx,wy),isoY(wx,wy));}
   c.stroke();}
  c.restore();
 ...
 for(const p of (G.map.patches||[])){
  if(p.towel){drawTowel(c,p);continue}   // v107.3: the bathroom's towel paints as cloth, not as a filled rectangle
...
/* v107.3: a dropped bath towel. It was a near-white rectangle with a thin
   outline, which on the v107.2 porcelain floor read as a blank white slab -
   the owner's note. A towel is cloth: it has a COLOUR, woven bands across each
   end, a fold with the far half catching more light, a hem and a shadow on the
   floor.
   ONE COLOUR, and it is chosen AGAINST THE FLOOR rather than rolled. The first
   cut picked a hue off dth from the towel's position, out of {teal, sand, blue}
   - and sand on v107.2's warm cream porcelain is the washed-out slab this
   release exists to fix, drawn again in a different hue (it came up on three
   seeds in four). TOWEL_HUE is a cool teal because the floor is warm, and
   T92.B asserts the CONTRAST rather than the colour, so a future palette change
   has to keep the towel readable and not merely keep this number. Rolling it
   would also have consumed the terrain stream and moved every decoration
   painted after it - the v97 deterministic-detail rule. */
const TOWEL_HUE=188;
function drawTowel(c,p){
 const hue=TOWEL_HUE;
 const sub=(x,y,w,h)=>({x:p.x+x,y:p.y+y,w,h});
 c.save();
 // the shadow it casts on the tile
 c.fillStyle='rgba(60,54,44,.18)';pathRegion(c,sub(.35,.45,p.w,p.h));c.fill();
 // the body, and the fold: the far half sits a shade lighter
 c.fillStyle=`hsl(${hue},26%,63%)`;pathRegion(c,p);c.fill();
 c.fillStyle=`hsla(${hue},30%,74%,.85)`;pathRegion(c,sub(0,0,p.w,p.h*.46));c.fill();
 // woven bands across each end, the way a bath towel is made
 c.fillStyle=`hsla(${hue},34%,46%,.55)`;
 for(const oy of [p.h*.12,p.h*.20,p.h*.78,p.h*.86]){pathRegion(c,sub(.5,oy,p.w-1,p.h*.045));c.fill();}
 // terry pile: short deterministic strokes, clipped to the cloth
 c.save();pathRegion(c,p);c.clip();c.lineCap='round';c.lineWidth=1.1;
 for(let i=0;i<Math.round(p.w*p.h*5);i++){
  const wx=p.x+dth(i*1.7+p.x)*p.w,wy=p.y+dth(i*3.1+p.y)*p.h,sx=isoX(wx,wy),sy=isoY(wx,wy);
  c.strokeStyle=dth(i*5.9)<.5?`hsla(${hue},30%,86%,.4)`:`hsla(${hue},30%,40%,.3)`;
  c.beginPath();c.moveTo(sx,sy);c.lineTo(sx+(dth(i*7.3)-.5)*2.4,sy-1.4-dth(i*11.1)*1.8);c.stroke();}
 c.restore();
 // the fold's own crease, and the hem all the way round
 c.strokeStyle=`hsla(${hue},32%,40%,.5)`;c.lineWidth=2;
 c.beginPath();c.moveTo(isoX(p.x,p.y+p.h*.46),isoY(p.x,p.y+p.h*.46));c.lineTo(isoX(p.x+p.w,p.y+p.h*.46),isoY(p.x+p.w,p.y+p.h*.46));c.stroke();
 c.strokeStyle=`hsla(${hue},34%,38%,.85)`;c.lineWidth=2.4;pathRegion(c,p);c.stroke();
 c.strokeStyle=`hsla(${hue},28%,82%,.6)`;c.lineWidth=1.2;pathRegion(c,sub(.55,.55,p.w-1.1,p.h-1.1));c.stroke();
 c.restore();
}
