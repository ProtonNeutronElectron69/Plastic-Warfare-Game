...
#
#   ./sim.sh                 8 matches, 2 on each of the 4 deathmatch maps
#   ./sim.sh 16              16 matches, 4 on each
...
#
# One match is ~30-70s of wall time. The default 8 take about two minutes on four
# cores. There are only FOUR deathmatch maps - desk is survOnly, wave-defense
# only - so the per-map count is the batch size over four.
set -e
N="${1:-8}"
SEED0="${SEED0:-101}"
JOBS="${JOBS:-$(nproc 2>/dev/null || echo 4)}"
MAPS_DM="backyard kitchen livingroom sandbox"
